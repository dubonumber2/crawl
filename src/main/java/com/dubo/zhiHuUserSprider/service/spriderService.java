package com.dubo.zhiHuUserSprider.service;


import com.dubo.zhiHuUserSprider.bean.ZhiHuUserBean;
import com.dubo.zhiHuUserSprider.bloomFilter.BloomFilter;
import com.dubo.zhiHuUserSprider.service.util.DBStatement;
...
import static com.dubo.zhiHuUserSprider.service.util.GetFromUrl.getFromUrl;
...
public class spriderService {
    ...
    public static void main(String[] args) throws InterruptedException {
        //从我的主页开始爬
       urlQueue.put("https://www.zhihu.com/people/liang-xing-yang");
       // urlQueue.put("https://www.zhihu.com/people/liangbianyao");
        ...
        for (int i=0;i<5;i++){
            ...
            a.setUncaughtExceptionHandler(new ErrHandler());
            executor.execute(a);
        }
...
//                    user.setAnswersNum(userContent.substring(userContent.indexOf("回答")+2,userContent.indexOf("提问")-1));
                    ...
                    // 被赞同数
                     ...
                     boolean tj1=e1.size()==3||(e1.size()==2&&e1.first().text().contains("知乎收录"));
                    if(tj1){
                        String starsNumString=e1.get(1).text();
                        starsNum=starsNumString.substring(starsNumString.indexOf("得")+2,starsNumString.indexOf("次")-1);
                    }
                    boolean tj2=(e1.size()==2||e1.size()==1)&&e1.first().text().contains("获得");
                    if(tj2){
                        String starsNumString=e1.first().text();
                        starsNum=starsNumString.substring(starsNumString.indexOf("得")+2,starsNumString.indexOf("次")-1);
                    }
                    user.setStarsNum(starsNum);
        ...
        String thxNum="0";
        String thxNumString=null;

            thxNumString = e2.get(e2.size() - 1).text();
            thxNum = thxNumString.substring(thxNumString.indexOf("得") + 2, thxNumString.indexOf("次") - 1);


                    if(null==thxNum||"".equals(thxNum)||"".equals(thxNum)){
                        user.setThxNum("0");
                    }else{
                        user.setThxNum(thxNum);
                    }
                    user.setStarsNum(starsNum);
        ...
        String followingNum="0";
                                    ...
                                    followingNum = userUrlContent.select(".NumberBoard-value").first().text();


                        user.setFollowingNum(followingNum);
                    //关注者数量
        String followersNum=null;
            followersNum = userUrlContent.select(".NumberBoard-value").get(1).text();
        user.setFollowersNum(followersNum);
                    ...
                    String sql = "insert into zhihuuser" +
                            "(name,sex,business,company,position,education,major,answersNum,starsNum,thxNum,followingNum,followersNum,createTime) " +
                            "values (?,?,?,?,?,?,?,?,?,?,?,?,now())";
                    try {
