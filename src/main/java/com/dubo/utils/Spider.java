...
import com.dubo.entity.ZhiHuBean;
import com.dubo.entity.ZhiHuPicBean;
...
public class Spider {
	...
	public static void getTopUserInfo(String url) throws Exception {
		...
		for (page = 1;page<=50;page++){
			...
			List<String> userUrls = ZhiHuUtil.getUserUrl(nowUrl);
			// 循环访问链接并获取用户信息
			for(String userUrl:userUrls){
				//初始化呢赋值
				User user = new User(baseUrl+userUrl);
				//打印当前用户信息
	...
	public static void downloadCommend(String url){
		 // 访问链接并获取页面内容
		 String content = ZhiHuUtil.SendGet(url);
		 // 获取该页面的所有的知乎对象
		 ArrayList<ZhiHuBean> myZhihu = ZhiHuUtil.GetRecommendations(content);
